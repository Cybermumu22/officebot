...
function parseArgs(argv) {
  ...
  const out = { _: [], port: envPort || DEFAULT_PORT, open: true, yes: false, host: process.env.AGENT_VIZ_HOST || '' };
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--port' || a === '-p') { out.port = parseInt(argv[++i], 10) || DEFAULT_PORT; }
    else if (a === '--host') { out.host = argv[++i] || ''; }
    else if (a === '--lan') { out.host = '0.0.0.0'; }   // friendly alias for "let my other devices view it"
...
function startServer(port, open, host) {
  process.env.AGENT_VIZ_PORT = String(port);
  if (host) process.env.AGENT_VIZ_HOST = host;   // else server.js defaults to 127.0.0.1
  ...
  if (host === '0.0.0.0' || host === '::' || host === 'all' || host === '*') {
    console.log('  ' + C.y('LAN mode: no login — anyone on your network can read your sessions. Trusted networks only.'));
  } else {
    console.log('  ' + C.dim('Private to this machine. To view from your phone/another device on a trusted'));
    console.log('  ' + C.dim('network: officebot --lan  (there is no authentication in LAN mode).'));
  }
...
function help() {
  console.log([
    ...
    '    --lan        allow other devices on your network to view it (no login —',
    '                 trusted networks only; default is private to this machine)',
    '    --host <ip>  bind to a specific address (advanced; --lan = 0.0.0.0)',
...
function main() {
  ...
  if (cmd === 'setup') {
    ...
    return startServer(args.port, args.open, args.host);
  }
  ...
  if (cmd === 'demo') { runDemo(args.port); return startServer(args.port, args.open, args.host); }
  if (cmd === 'start') return startServer(args.port, args.open, args.host);
