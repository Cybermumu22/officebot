...
(pixel art from the bundled roster in `public/avatars.js`, glowing
...
**CONVO chat API (2026-07): `GET /api/chat?tab=deck-N&from=<bytes>&tpath=<path>`.**
Serves the Pocket Deck's chat pane. Finds the tab's session by scanning
`sessionCache` for `lastEvent.deckTab` (newest wins, live beats ended), then
reads that session's transcript JSONL **incrementally by byte offset** — the
file is append-only, so `from` (returned each call) lets a 2s poll read only
appended bytes; only complete lines are parsed (a `\n` byte never occurs
inside a multibyte UTF-8 sequence, so offsets are always valid boundaries).
A `tpath` mismatch (new session, `/clear`, `--resume`) or a shrunken file
forces a fresh 512KB tail with `reset:true`. Parsing whitelists
`type user|assistant`, skips `isSidechain`/`isMeta` and slash-command noise
(`<command-name>`/`<local-command-stdout>`/`<local-command-caveat>`), joins
assistant text blocks and **merges the multiple JSONL lines that share one
`message.id`** (a single logical reply spans up to ~7 lines); messages carry
`id` so the client merges across polls too. The response's `status`
(`working`/`waiting`/`idle`/`ended`/`none` + an activity label derived from
the last hook's tool_name/tool_input) drives the pane's ticker; `Notification`
was added to the installed hooks for the `waiting` state. Dialog detection is
...
`officebot.welcomed`) and rebuilt on load. Before this, both feeds were
DOM-only — the deck's auto-refresh on app-return wiped them, so the crew's
whole going-home conversation (played by the ghost-buster while you were
away) vanished right before you could read it, and the office looked like
it had emptied silently. The dedupe guards (`_lastChatKey`/`_lastMoment`)
are seeded from the newest restored line so a replay can't double-log it.

**Coming back after going quiet is a ceremony, not a pop (2026-07).** The
client ghost-buster (12 silent minutes) walks the office out and tombstones
the session — but as a *resurrectable* `'quiet'` tombstone, unlike a real
`SessionEnd`'s permanent one. A fresh LIVE work hook (prompt/tool —
`REAL_WORK_HOOKS`; never SpeechTick/ModelTick, which dead sessions keep
emitting) clears it, and a live work hook landing on an office that is
EMPTY re-runs the arrival ceremony even for an already-`_welcomed` session
(rushed automatically, so the crew hustles in through the door instead of
teleporting to their desks — that instant-materialize was the old,
immersion-breaking behaviour). A live `SessionStart` (relaunch, `--resume`,
Pocket Deck's opener wrapper) on an empty/drained office gets the same
treatment — its case-handler's `_welcomed` skip now only applies to
replayed SessionStarts. Snapshot playback is excluded via a server-side
`_replay: 1` flag stamped in `replaySnapshot`, which is what keeps the
original rule intact: a reload over a live office still staffs instantly,
no theatre.
...
it's ever seen until a real `SessionEnd` (or the 15-minute stale-session
sweep, `STALE_SESSION_MS`) clears it. Test/demo traffic that never sends `SessionEnd` — like
...
2. **`GET /snapshot.png`** — a recent PNG of the first office, rendered by
   `snapshot.js` (headless Chromium via whatever Playwright is installed
   globally or locally, else the `AGENT_VIZ_PLAYWRIGHT` env var pointing at
   one; officebot itself stays zero-dep). Spawned LAZILY by the server on
   first request and self-exits
